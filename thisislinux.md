...
* /etc/skel : 처음 유저가만들어 질때 유저의 디렉토리에 기본으로 생성되는 파일들이 모여있는 곳이다.
...
* systemctl start/stop/restart/status networking : 네트워크 설정을 변경한 후에 변경된 내용을 시스템에 적용시키는 명령어

* nslookup : DNS 서버의 작동을 테스트하는 명령어

* ping IP 주소 또는 URL : 해당 컴퓨터가 네트워크상에서 응답하는지 테스트하는 명령어
...
* 프로세스 : '하드디스크에 저장된 실행 코드(프로그램)가 메모리에 올라가 활성화 된 상태'
    ...
    * 백그라운드 프로세스 : 실행은 되었지만,  화면에 나타나지 않고 뒤에서 실행되는 프로세스
         * gedit & : gedit을 백그라운드로 프로세스로 실행하겠다는 의미, 명령어 뒤에 &를 붙이면 백그라운드로 실행하겠다는 의미이다.
         * 백그라운드 프로세스는 별도의 창이 열리는(firefox, gedit) 것들에는 상관없지만 vi 에디터와 같이 현재 터미널에서 입력해야 하는 것들에는 화면에 입출력을 할 수 없으니 의미가 없다.
...
* standalone : ftp 데몬이 독립적으로 작동하는 방식으로, 평소에도 늘 ftp 서비스가 메모리에 상주하고 있다. 자원의 낭비는 있겠지만 ftp 서비스 요청시
즉각 반응하는 장점이 있다.
